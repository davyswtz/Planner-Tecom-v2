...
class CorrecaoDeSinalController extends Controller
{
    ...
    public function show(OpTask $correcao_sinal): JsonResponse
    {
        if (!$correcao_sinal->isCorrecaoSinalPai()) {
            return response()->json(
                ["message" => "Correção de sinal não encontrada"],
                404,
            );
        }
        ...
        return response()->json([
            ...
            "correcaoDeSinal" => $this->correcaoDeSinalService->normalizarParaExibicao(
                $correcao_sinal,
            ),
        ]);
    }
    ...
    {
        $correcao = $this->correcaoDeSinalService->createCorrecaoDeSinal(
            $request->all(),
        );
    ...
    public function storeFromCaixa(Request $request): JsonResponse
    {
        $request->validate([
            ...
            "sinal_rx_olt" => ["nullable", "numeric"],
            "codigo_cliente" => ["nullable"],
            "id_cliente_servico" => ["nullable"],
            "regiao" => ["nullable", "string", "max:120"],
        ]);

        $correcao = $this->correcaoDeSinalService->createFromClienteCaixa(
            $request->all(),
        );
    ...
    public function update(
        ...
        $resultado = $this->correcaoDeSinalService->updateCorrecaoDeSinal(
            $correcao_sinal,
            $request->all(),
        );
    ...
    public function destroy(OpTask $correcao_sinal): JsonResponse
    {
        ...
        return response()->json([
            "message" => "Correção de sinal deletada com sucesso",
        ]);
    }

    public function buscarEndereco(Request $request): JsonResponse
    {
        $endereco = $this->correcaoDeSinalService->buscarEndereco(
            (string) $request->query("coordenada", ""),
        );
