...
class OrdemServicoService
{
    ...
    private function contarPorStatus(
        Collection $items,
        string $statusNormalizado,
    ): int {
        return $items
            ->filter(
                fn(array $item) => $this->normalizarStatusOs(
                    $item["status"] ?? "",
                ) === $statusNormalizado,
            )
            ->count();
    ...
    public function getDashboard(array $filtros = []): array
    {
        ...
        $porTecnico = $os
            ...
            ->map(function (Collection $items, string $tecnico) {
                $regiao =
                    $items->pluck("tecnico_regiao")->filter()->first() ?:
                    $items
                            ->pluck("regiao")
                            ->filter()
                            ->countBy()
                ...
                return [
                    ...
                    "em_andamento" => $this->contarPorStatus(
                        $items,
                        "Em andamento",
                    ),
                    "finalizada" => $this->contarPorStatus(
                        $items,
                        "Finalizada",
                    ),
                    "total" =>
                        $this->contarPorStatus($items, "Aberta") +
        ...
        $porCategoriaPai = $osUnicas
            ->groupBy(
                fn(array $item) => $this->rotuloCategoriaPai(
                    $item["categoria_pai"],
                ),
            )
        ...
        // Adicionar por_prioridade (apenas OS abertas/em andamento)
        $porPrioridade = $osUnicas
            ->filter(
                fn(array $item) => in_array($item["status"], [
                    "Aberta",
                    "Em andamento",
                ]),
            )
            ->groupBy(fn(array $item) => $item["prioridade"] ?: "Média")
            ->map(
                fn(Collection $items, string $prioridade) => [
        ...
        return [
            "totais" => [
                "total" =>
                    $this->contarPorStatus($os, "Aberta") +
    ...
    private function descreverFiltrosExportacao(array $filtros): array
    {
        ...
        if (!empty($filtros["dataInicio"]) || !empty($filtros["dataFim"])) {
            $tipo =
                ($filtros["tipoData"] ?? "criacao") === "conclusao"
                    ? "Data de conclusão"
                    : "Data de criação";
    ...
    private function listarOsNormalizadas(array $filtros = []): Collection
    {
        ...
        $normalizadas = $tasks->flatMap(function (OpTask $task) use (
            $osTecnicoPorTask,
            $parentsPorId,
        ) {
            ...
            $parentId =
                $task->parent_task_id ?: $registro->parent_task_id ?? null;
            $parentTask = $parentId
                ? $parentsPorId->get((int) $parentId)
                : null;
            ...
            $base = [
                "id" => (int) $task->id,
                "regiao" =>
                    trim((string) $task->regiao) ?:
                    trim((string) ($registro->regiao ?? "")),
                "status" => $this->normalizarStatusOs(
                    $task->status ?: $registro->status ?? "",
                ),
                "status_raw" => $task->status ?: $registro->status ?? "",
                "criadaEm" => $task->criadaEm,
                "data_criacao" => $this->resolverDataCriacao($task, $registro),
                "data_conclusao" => $this->resolverDataConclusao(
                    $task,
                    $registro,
                ),
                "titulo" =>
                    trim((string) $task->titulo) ?:
                    trim((string) ($registro->titulo ?? "")),
                "numero_os" => trim(
                    (string) ($task->numero_os ?:
                    $task->ordem_servico ?:
                    $registro->ordem_servico ?? ""),
                ),
                "ordem_servico" => trim(
                    (string) ($task->ordem_servico ?:
                    $registro->ordem_servico ?? ""),
                ),
                "taskCode" =>
                    trim((string) $task->taskCode) ?:
                    trim((string) ($registro->task_code ?? "")),
                "categoria" =>
                    trim((string) $task->categoria) ?:
                    trim((string) ($registro->categoria ?? "")),
                "categoria_pai" =>
                    $parentTask?->categoria ??
                    trim((string) $task->sub_processo),
                "categoria_pai_label" => $this->rotuloCategoriaPai(
                    $parentTask?->categoria ??
                        trim((string) $task->sub_processo),
                ),
                "task_code_pai" => $parentTask?->taskCode ?? "",
                "prioridade" =>
                    trim((string) $task->prioridade) ?:
                    trim((string) ($registro->prioridade ?? "")) ?:
                    "Média",
                "parent_task_id" => $parentId ? (int) $parentId : null,
                "descricao" => $task->descricao,
                "protocolo" =>
                    trim((string) $task->protocolo) ?:
                    trim((string) ($registro->protocolo ?? "")),
            ...
            return collect($tecnicos)->map(function (string $tecnico) use (
                $base,
            ) {
                $meta = $this->tecnicoNomeResolver->resolverOuOriginal(
                    $tecnico,
                );
        ...
        $somenteIdenticas = $normalizadas
            ->filter(fn(array $item) => $item["tecnico_identificado"] ?? false)
            ->values();

        return $this->aplicarFiltrosPosQuery($somenteIdenticas, $filtros);
    }
    ...
    private function aplicarFiltrosQuery(Builder $query, array $filtros): void
    {
        ...
        if (!empty($filtros["regiao"])) {
            ...
            $query->where(function (Builder $q) use ($regiao) {
                $q->where("op_tasks.regiao", $regiao)->orWhereIn(
                    "op_tasks.id",
                    OsTecnico::query()
                        ->select("task_id")
                        ->where("regiao", $regiao)
        ...
        if (!empty($filtros["prioridade"])) {
            ...
            $query->where(function (Builder $q) use ($prioridade) {
                $q->where("op_tasks.prioridade", $prioridade)->orWhereIn(
                    "op_tasks.id",
                    OsTecnico::query()
                        ->select("task_id")
        ...
        if (!empty($filtros["categoriaPai"])) {
            ...
            $query->where(function (Builder $q) use ($categoria) {
                $q->whereHas(
                    "parentTask",
                    fn(Builder $parent) => $parent->where(
                        "categoria",
                        $categoria,
                    ),
                )->orWhereIn(
    ...
    private function aplicarFiltrosPosQuery(
        Collection $items,
        array $filtros,
    ): Collection {
        if (!empty($filtros["tecnico"])) {
            ...
            $items = $items->filter(
                fn(array $item) => mb_strtolower($item["tecnico"]) === $tecnico,
            );
        }
        ...
        if (!empty($filtros["status"])) {
            ...
            $items = $items->filter(
                fn(array $item) => $item["status"] === $status,
            );
        }
        ...
        if (!empty($filtros["dataInicio"]) || !empty($filtros["dataFim"])) {
            ...
            $items = $items->filter(function (array $item) use (
                $campo,
                $inicio,
                $fim,
            ) {
    ...
    private function resolverDataCriacao(
        OpTask $task,
        ?OsTecnico $registro,
    ): ?string {
    ...
    private function resolverDataConclusao(
        OpTask $task,
        ?OsTecnico $registro,
    ): ?string {
    ...
    private function resolverTecnicos(
        OpTask $task,
        Collection $registrosOsTecnico,
    ): array {
        $nomesOsTecnico = $registrosOsTecnico
            ->map(
                fn(OsTecnico $registro) => trim(
                    (string) $registro->tecnico_nome,
                ),
            )
        ...
        $nomesResponsavel =
            $responsavel !== "" ? OpTask::parseResponsaveis($responsavel) : [];
    ...
    /**
     * Retorna dados para o gráfico de heatmap de atividade.
     */
    public function getHeatmap(
        string $tipo = "mensal",
        ?string $mes = null,
    ): array {
        $mes = preg_match('/^\d{4}-\d{2}$/', (string) $mes)
            ? (string) $mes
            : now()->format("Y-m");

        $inicioMes = \Carbon\Carbon::createFromFormat(
            "Y-m-d",
            $mes . "-01",
        )->startOfMonth();
        ...
        if ($tipo === "semanal") {
            $dataReferencia =
                $mes === now()->format("Y-m") ? now() : $inicioMes->copy();
            $inicioSemana = $dataReferencia
                ->copy()
                ->startOfWeek(\Carbon\Carbon::SUNDAY);
            $fimSemana = $inicioSemana->copy()->addDays(6);

            $osSemana = $this->listarOsNormalizadas([
                "dataInicio" => $inicioSemana->toDateString(),
                "dataFim" => $fimSemana->toDateString(),
                "tipoData" => "criacao",
            ])
                ->unique("id")
                ->values();

            $regioesFixas = collect([
                "Goval",
                "Vale do Aço",
                "Sem região",
                "Caratinga",
            ...
            $porRegiaoCalculada = $osSemana
                ->groupBy(fn(array $item) => $item["regiao"] ?: "Sem região")
                ->map(
                    fn(Collection $items, string $regiao) => [
                        "regiao" => $regiao,
                        "dias" => $this->calcularEficienciaPorDia($items),
                    ],
                );

            $porRegiao = $regioesFixas
                ->map(
                    fn(string $regiao) => $porRegiaoCalculada->get($regiao) ?? [
                        "regiao" => $regiao,
                        "dias" => [],
                    ],
                )
                ->merge(
                    $porRegiaoCalculada
                        ->reject(
                            fn(array $item) => $regioesFixas->contains(
                                $item["regiao"],
                            ),
                        )
                        ->sortBy(
                            fn(array $regiao) => mb_strtolower(
                                $regiao["regiao"],
                            ),
                        )
                        ->values(),
        ...
        $osMes = $this->listarOsNormalizadas([
            "dataInicio" => $inicioMes->toDateString(),
            "dataFim" => $fimMes->toDateString(),
            "tipoData" => "criacao",
        ])
            ->unique("id")
            ->values();
        ...
        return [
            "tipo" => "mensal",
            "mes" => $mes,
            "dias" => $this->calcularEficienciaPorDia($osMes),
        ];
    }

    /**
     * Calcula a eficiência diária de uma coleção de OS
     * Fórmula: (Finalizadas * 100 + Em Andamento * 50) / Total
     */
    private function calcularEficienciaPorDia(Collection $os): array
    {
        return $os
            ->groupBy(
                fn(array $item) => substr($item["data_criacao"] ?? "", 0, 10),
            )
            ->filter(fn(Collection $items, string $dia) => $dia !== "")
            ->map(function (Collection $items, string $dia) {
                $abertas = $this->contarPorStatus($items, "Aberta");
                $andamento = $this->contarPorStatus($items, "Em andamento");
                $finalizadas = $this->contarPorStatus($items, "Finalizada");
                $total = $abertas + $andamento + $finalizadas;

                $eficiencia =
                    $total > 0
                        ? ($finalizadas * 100 + $andamento * 50) / $total
                        : 0;
