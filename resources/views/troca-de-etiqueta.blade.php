...
@section('btn-label', 'Nova troca de etiqueta')
...
<style>
  ...
  .etiquetas-input-box:focus-within {
    border-color: #166ac4;
    box-shadow: 0 0 0 3px rgba(22,106,196,0.12);
  }
  .etiquetas-chips { display: contents; }
  .etiqueta-chip {
  ...
  .etiqueta-chip-c0 { background: #dbeafe; color: #1e40af; border-color: #93c5fd; }
  .etiqueta-chip-c1 { background: #dcfce7; color: #166534; border-color: #86efac; }
  .etiqueta-chip-c2 { background: #fef3c7; color: #92400e; border-color: #fcd34d; }
  .etiqueta-chip-c3 { background: #ede9fe; color: #5b21b6; border-color: #c4b5fd; }
  .etiqueta-chip-c4 { background: #cffafe; color: #0e7490; border-color: #67e8f9; }
  .etiqueta-chip-c5 { background: #ffe4e6; color: #9f1239; border-color: #fda4af; }
  ...
  [data-theme="dark"] .etiqueta-chip-c0 { background: #0d2340; color: #93c5fd; border-color: #1e40af; }
  [data-theme="dark"] .etiqueta-chip-c1 { background: #0b2215; color: #86efac; border-color: #166534; }
  [data-theme="dark"] .etiqueta-chip-c2 { background: #201800; color: #fcd34d; border-color: #92400e; }
  [data-theme="dark"] .etiqueta-chip-c3 { background: #1a1033; color: #c4b5fd; border-color: #5b21b6; }
  [data-theme="dark"] .etiqueta-chip-c4 { background: #0c2328; color: #67e8f9; border-color: #0e7490; }
  [data-theme="dark"] .etiqueta-chip-c5 { background: #2a0e14; color: #fda4af; border-color: #9f1239; }
  .etiquetas-inline-input {
    flex: 1 1 140px;
    min-width: 140px;
    border: none; outline: none; background: transparent;
    font-size: 13px; font-family: inherit; color: var(--gray-950);
    padding: 4px 2px; height: 28px;
  }
  .etiquetas-inline-input::placeholder { color: var(--gray-400); }
  .btn-adicionar-etiqueta-inline {
  ...
  [data-theme="dark"] .etiqueta-loc-row { background: #0a0c10; border-color: #1e2228; }
  ...
  [data-theme="dark"] .etiqueta-detalhe-card { background: #0a0c10; border-color: #1e2228; }
  ...
  .etiqueta-chip-view.etiqueta-chip-c0 { background: #dbeafe; color: #1e40af; border-color: #93c5fd; }
  .etiqueta-chip-view.etiqueta-chip-c1 { background: #dcfce7; color: #166534; border-color: #86efac; }
  .etiqueta-chip-view.etiqueta-chip-c2 { background: #fef3c7; color: #92400e; border-color: #fcd34d; }
  .etiqueta-chip-view.etiqueta-chip-c3 { background: #ede9fe; color: #5b21b6; border-color: #c4b5fd; }
  .etiqueta-chip-view.etiqueta-chip-c4 { background: #cffafe; color: #0e7490; border-color: #67e8f9; }
  .etiqueta-chip-view.etiqueta-chip-c5 { background: #ffe4e6; color: #9f1239; border-color: #fda4af; }
  [data-theme="dark"] .etiqueta-chip-view.etiqueta-chip-c0 { background: #0d2340; color: #93c5fd; border-color: #1e40af; }
  [data-theme="dark"] .etiqueta-chip-view.etiqueta-chip-c1 { background: #0b2215; color: #86efac; border-color: #166534; }
  [data-theme="dark"] .etiqueta-chip-view.etiqueta-chip-c2 { background: #201800; color: #fcd34d; border-color: #92400e; }
  [data-theme="dark"] .etiqueta-chip-view.etiqueta-chip-c3 { background: #1a1033; color: #c4b5fd; border-color: #5b21b6; }
  [data-theme="dark"] .etiqueta-chip-view.etiqueta-chip-c4 { background: #0c2328; color: #67e8f9; border-color: #0e7490; }
  [data-theme="dark"] .etiqueta-chip-view.etiqueta-chip-c5 { background: #2a0e14; color: #fda4af; border-color: #9f1239; }
...
<!-- MODAL CRIAR -->
<x-modal id="modal-overlay" titulo="Nova Troca de Etiqueta" subtitulo="Preencha os dados da Troca de Etiqueta">
  ...
  <div class="modal-form">
    ...
    <div class="os-field">
      ...
      <select id="input-regiao" class="os-input" onchange="carregarTecnicos(this.value, 'input-tecnico')">
  ...
  <x-slot name="footer">
    ...
    <button onclick="criarTrocaEtiqueta()" class="btn-modal btn-modal-primary">
      <i class="ti ti-tag" style="font-size:14px"></i> Criar Troca de Etiqueta
    </button>
  </x-slot>
...
<x-modal
  ...
  <div id="detalhe-tab-os" style="display:none">
    ...
    <div class="os-empty">
      ...
      <span>Nenhuma OS vinculada a esta Troca de Etiqueta</span>
    </div>
  </div>

  <x-slot name="footer">
    <div class="modal-foot-inner">
      <button type="button" onclick="abrirConfirmacaoExclusao()" id="btn-excluir" class="btn-modal btn-modal-danger" title="Excluir esta Troca de Etiqueta">
...
<div id="confirm-excluir-overlay" class="confirm-excluir-overlay" role="dialog" aria-modal="true" aria-labelledby="confirm-excluir-title">
  <div class="confirm-excluir-box">
    ...
    <p class="confirm-excluir-title" id="confirm-excluir-title">Excluir Troca de Etiqueta?</p>
...
<div class="card" style="flex:1">
  <div class="card-header">
    <span class="card-title">Kanban de Troca de Etiqueta</span>
    <span class="card-action">total: <span id="total-etiquetas">0</span></span>
  </div>
  <div class="kanban-cols">
    <div class="kcol">
      ...
      <div class="kcol-content-wrap">
        <div class="kcol-body" id="col-pendente" data-status="Pendente"></div>
...
<script>
  ...
  async function aplicarFiltros() {
    if (window.carregarEtiquetas) {
      window.carregarEtiquetas(obterFiltrosFormulario());
    }
  }
  ...
  function parseEtiquetasTitulo(titulo) {
    return String(titulo || '')
      .split(/[,;]+/)
      .map(normalizarNomeEtiqueta)
      .filter(Boolean);
  }

  function focarInputEtiqueta(event) {
    if (event.target.closest('.etiqueta-chip-remove, .btn-adicionar-etiqueta-inline')) return;
    document.getElementById('input-etiqueta-nome')?.focus();
  }
  window.focarInputEtiqueta = focarInputEtiqueta;

  function handleEtiquetaInputKeydown(event) {
    if (event.key === 'Enter') {
  ...
  window.handleEtiquetaInputKeydown = handleEtiquetaInputKeydown;

  function escEtiqueta(valor) {
    return String(valor || '')
      .replace(/&/g, '&amp;')
      .replace(/</g, '&lt;')
      .replace(/>/g, '&gt;')
  ...
  function classeCorEtiqueta(index) {
  ...
  function adicionarEtiquetaNome(event) {
    ...
    const bruto = normalizarNomeEtiqueta(input.value);
    if (!bruto) return;
  ...
  window.adicionarEtiquetaNome = adicionarEtiquetaNome;
...
window.alterarStatusOS = async function(osId, novoStatus) {
  ...
  async function carregarOS(trocaId) {
    const token = localStorage.getItem('planner_token');
    const response = await fetch(`/api/troca-etiqueta/${trocaId}/os`, {
      headers: {
        'Authorization': 'Bearer ' + token,
        'Accept': 'application/json',
      },
  ...
  // A tarefa pai (Troca de Etiqueta) permanece; apenas a OS vinculada é removida.
  ...
  async function confirmarExclusaoOs() {
    ...
    try {
      ...
      await window.plannerAposMutacaoLocal(async () => {
        ...
        if (window.carregarEtiquetas) await window.carregarEtiquetas();
      });
    } catch (err) {
  ...
  async function confirmarExclusaoTarefa() {
    ...
    try {
      ...
      if (!response.ok) {
        const erro = await response.json();
        throw new Error(erro.message || 'Erro ao excluir Troca de Etiqueta.');
      }

      fecharConfirmacaoExclusao();
      fecharDetalhe();
      await window.plannerAposExclusaoTarefa(id, () => window.carregarEtiquetas());
    } catch (err) {
      console.error('Erro ao excluir Troca de Etiqueta:', err.message);
...
document.addEventListener('keydown', function(e) {
  ...
  async function carregarTecnicos(regiao, destino = 'os-input-tecnico') {
    ...
    if (regiao && tecnicos.length === 0 && (destino === 'os-input-tecnico' || destino === 'input-tecnico')) {
  ...
  async function criarTrocaEtiqueta() {
    ...
    if (document.getElementById('input-etiqueta-nome')?.value.trim()) {
      adicionarEtiquetaNome();
    }
    ...
    if (!titulo) {
      alert('Informe ao menos uma etiqueta.');
      return;
    }
    ...
    const token = localStorage.getItem('planner_token');
    const response = await fetch('/api/troca-etiqueta', {
      method: 'POST',
    ...
    if (response.ok) {
      ...
      alert(resultado.message || 'Erro ao criar Troca de Etiqueta.');
      console.error('Erro ao criar Troca de Etiqueta:', resultado);
    }
  }
  ...
  async function buscarEndereco(coordenada, index = null) {
    ...
    const response = await fetch(
      `/api/troca-etiqueta/coordenada?coordenada=${encodeURIComponent(coord)}`,
...
<script type="module">
  ...
  const offsetMap = { 'Pendente': 0, 'Em andamento': 0, 'Impedimento': 0, 'Concluída': 0 };
  const limitMap  = { 'Pendente': 10, 'Em andamento': 10, 'Impedimento': 10, 'Concluída': 50 };
  const colIdMap  = { 'Pendente': 'pendente', 'Em andamento': 'andamento', 'Impedimento': 'impedimento', 'Concluída': 'concluida' };
  ...
  async function carregarMais(status) {
    ...
    novos.forEach(r => { col.insertAdjacentHTML('beforeend', renderCard(r)); etiquetasMap[r.id] = r; });
  ...
  function badgeStatus(status) {
    const exibicao = status === 'Backlog' || status === 'Criada' ? 'Pendente'
      : status === 'Concluída' || status === 'Finalizada' || status === 'Finalizado' ? 'Concluída'
      : (status || '—');
    const mapa = { 'Pendente':'d-blue', 'Em andamento':'d-amber', 'Impedimento':'d-red', 'Concluída':'d-green' };
  ...
  async function buscarColuna(status, limit, offset = 0, filtros = {}) {
    ...
    const response = await fetch(`/api/troca-etiqueta?${params}`, {
      headers: { 'Authorization': 'Bearer ' + token, 'Accept': 'application/json' },
      cache: 'no-store',
    });
    const data = await response.json();
    const lista = data.trocaDeEtiqueta || [];
    return window.plannerFiltrarExcluidas ? window.plannerFiltrarExcluidas(lista) : lista;
  }

  async function carregarEtiquetas(filtros) {
    ...
    const [pendentes, andamento, impedimento, concluidas] = await Promise.all([
      buscarColuna('Pendente', 10, 0, filtrosAtivos),
      buscarColuna('Em andamento', 10, 0, filtrosAtivos),
      buscarColuna('Impedimento', 10, 0, filtrosAtivos),
      buscarColuna('Concluída', 50, 0, filtrosAtivos),
    ]);

    const todos = [...pendentes, ...andamento, ...impedimento, ...concluidas];
    if (window.plannerIsReloadCurrent && !window.plannerIsReloadCurrent(gen)) return;

    Object.keys(etiquetasMap).forEach(k => delete etiquetasMap[k]);
    todos.forEach(r => { etiquetasMap[r.id] = r; });

    document.getElementById('col-pendente').innerHTML      = pendentes.map(renderCard).join('');
    ...
    document.getElementById('col-concluida').innerHTML  = concluidas.map(renderCard).join('');
    ...
    document.getElementById('count-concluida').textContent  = concluidas.length;
    document.getElementById('total-etiquetas').textContent = todos.length;
    ...
    document.getElementById('mais-concluida').style.display  = concluidas.length === 50 ? 'block' : 'none';
  }
  window.carregarEtiquetas = carregarEtiquetas;

  // ─── RENDER CARD ───
  function renderCard(r) {
    const regiaoClass = r.regiao && r.regiao.toLowerCase().includes('vale') ? 'b-regiao-va' : 'b-regiao-gv';
    const codigo = r.taskCode || '';
    const etiquetas = parseEtiquetasTitulo(r.titulo);
    const etiquetasHtml = etiquetas.length
      ? etiquetas.map((n, i) => `<span class="etiqueta-chip-view ${classeCorEtiqueta(i)}">${esc(n)}</span>`).join('')
      : `<span class="etiqueta-chip-view etiqueta-chip-c0">${esc(r.titulo || '—')}</span>`;
    return `
    <div class="kcard"
      data-id="${r.id}"
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px">
        <span class="kcard-code" style="font-size:11px">${codigo || (etiquetas[0] || 'S/C')}</span>
      </div>
      <div class="etiquetas-view-chips" style="margin-bottom:4px">${etiquetasHtml}</div>
  ...
  function renderDetalhe(r) {
    const etiquetas = parseEtiquetasTitulo(r.titulo);
    document.getElementById('detalhe-titulo').textContent = etiquetas.length
      ? `${etiquetas.length} etiqueta${etiquetas.length > 1 ? 's' : ''}`
      : (r.titulo || 'Troca de Etiqueta');
    ...
    document.getElementById('detalhe-conteudo').innerHTML = `
      <div style="display:flex;flex-direction:column;gap:16px" class="detail-enter">
        ...
        <div class="detail-grid-2">
          ...
          ${campoDetalhe('Técnico responsável', esc(r.responsavel), 1, 'campo-responsavel')}
        </div>
        <div class="detail-grid-2">
    ...
    document.getElementById('detalhe-conteudo').dataset.tituloEtiquetas = r.titulo || '';
    carregarOS(r.id);
  }
  ...
  async function abrirDetalhe(id) {
    ...
    try {
      ...
      renderDetalhe(data.trocaDeEtiqueta || data.troca || data);
    } catch {
