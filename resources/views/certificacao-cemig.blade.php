...
@section('title', 'Certificação — Planner Telecom')
@section('page-title', 'Certificação')
...
<!-- MODAL CRIAR -->
<x-modal id="modal-overlay" titulo="Nova certificação Cemig" subtitulo="Preencha os dados da certificação">

  <div class="modal-form">

    <div class="os-field">
      <label class="os-label">Título / Notificação</label>
      <input type="text" id="input-titulo" placeholder="Ex: Cemig — NOTIFICAÇÃO ER/EE-00202/2026:EE" class="os-input"/>
    </div>
    ...
    <div class="os-field">
      ...
      <textarea id="input-descricao" rows="3" placeholder="Detalhes adicionais (opcional)" class="os-input"
    ...
    <div class="os-field">
      ...
      <div class="prioridade-wrap" role="group" aria-label="Prioridade">
        <button type="button" onclick="selecionarPrioridade(this,'Baixa')" class="btn-prioridade btn-prio-baixa" data-nivel="Baixa" aria-pressed="false"><span class="prio-label">Baixa</span></button>
        <button type="button" onclick="selecionarPrioridade(this,'Média')" class="btn-prioridade btn-prio-media btn-prio-ativo" data-nivel="Média" aria-pressed="true"><span class="prio-label">Média</span></button>
        <button type="button" onclick="selecionarPrioridade(this,'Alta')" class="btn-prioridade btn-prio-alta" data-nivel="Alta" aria-pressed="false"><span class="prio-label">Alta</span></button>
      </div>
    </div>
  ...
  <x-slot name="footer">
    ...
    <button onclick="criarCertificacao()" class="btn-modal btn-modal-primary">
      <i class="ti ti-certificate" style="font-size:14px"></i> Criar certificação
    </button>
  </x-slot>
...
<x-modal
  ...
  <div id="detalhe-tab-os" style="display:none">
    ...
    <div class="os-empty">
      ...
      <span>Nenhuma OS vinculada a esta certificação</span>
    </div>
  </div>

  <x-slot name="footer">
    <div class="modal-foot-inner">
      <button type="button" onclick="abrirConfirmacaoExclusao()" id="btn-excluir" class="btn-modal btn-modal-danger" title="Excluir esta certificação">
...
<div id="confirm-excluir-overlay" class="confirm-excluir-overlay" role="dialog" aria-modal="true" aria-labelledby="confirm-excluir-title">
  <div class="confirm-excluir-box">
    ...
    <p class="confirm-excluir-title" id="confirm-excluir-title">Excluir certificação?</p>
    <p class="confirm-excluir-text" id="confirm-excluir-text">
      Esta ação não pode ser desfeita. A certificação e os dados vinculados serão removidos permanentemente.
    </p>
...
<div class="card" style="flex:1">
  ...
  <div class="kanban-cols kanban-cols-5">
    ...
    <div class="kcol">
      ...
      <div class="kcol-content-wrap">
        <div class="kcol-body" id="col-validacao" data-status="Validação"></div>
    ...
    <div class="kcol">
      ...
      <div class="kcol-content-wrap">
        <div class="kcol-body" id="col-adequacao" data-status="Precisa de adequação"></div>
...
<script>
  ...
  async function aplicarFiltros() {
    if (window.carregarCertificacoes) {
      window.carregarCertificacoes(obterFiltrosFormulario());
    }
  }
...
window.alterarStatusOS = async function(osId, novoStatus) {
  ...
  async function carregarOS(correcaoId) {
    const token = localStorage.getItem('planner_token');
    const response = await fetch(`/api/certificacao-cemig/${correcaoId}/os`, {
      headers: {
        'Authorization': 'Bearer ' + token,
        'Accept': 'application/json',
      },
  ...
  // A tarefa pai (certificação) permanece; apenas a OS vinculada é removida.
  ...
  async function confirmarExclusaoOs() {
    ...
    try {
      ...
      await window.plannerAposMutacaoLocal(async () => {
        ...
        if (window.carregarCertificacoes) await window.carregarCertificacoes();
      });
    } catch (err) {
  ...
  async function confirmarExclusaoTarefa() {
    ...
    try {
      ...
      if (!response.ok) {
        const erro = await response.json();
        throw new Error(erro.message || 'Erro ao excluir certificação.');
      }
...
document.addEventListener('keydown', function(e) {
  ...
  async function criarCertificacao() {
    ...
    if (!titulo) {
      alert('Informe o título ou notificação.');
      return;
    }
    if (!regiao) {
    ...
    const token = localStorage.getItem('planner_token');
    const response = await fetch('/api/certificacao-cemig', {
      method: 'POST',
    ...
    if (response.ok) {
      ...
      alert(resultado.message || 'Erro ao criar certificação.');
      console.error('Erro ao criar certificação:', resultado);
    }
  }
...
<script type="module">
  ...
  const certificacoesMap = {};
  let filtrosAtivos = {};
  const isTouchDevice = window.matchMedia('(pointer: coarse)').matches;

  const offsetMap = { 'Pendente': 0, 'Em andamento': 0, 'Validação': 0, 'Precisa de adequação': 0, 'Concluído': 0 };
  const limitMap  = { 'Pendente': 10, 'Em andamento': 10, 'Validação': 10, 'Precisa de adequação': 10, 'Concluído': 50 };
  const colIdMap  = { 'Pendente': 'pendente', 'Em andamento': 'andamento', 'Validação': 'validacao', 'Precisa de adequação': 'adequacao', 'Concluído': 'concluido' };
  ...
  async function buscarColuna(status, limit, offset = 0, filtros = {}) {
    ...
    const response = await fetch(`/api/certificacao-cemig?${params}`, {
  ...
  async function carregarCertificacoes(filtros) {
    ...
    const [pendentes, andamento, validacao, adequacao, concluidos] = await Promise.all([
      buscarColuna('Pendente', 10, 0, filtrosAtivos),
      buscarColuna('Em andamento', 10, 0, filtrosAtivos),
      buscarColuna('Validação', 10, 0, filtrosAtivos),
      buscarColuna('Precisa de adequação', 10, 0, filtrosAtivos),
      buscarColuna('Concluído', 50, 0, filtrosAtivos),
    ]);
    ...
    document.getElementById('col-validacao').innerHTML  = validacao.map(renderCard).join('');
    document.getElementById('col-adequacao').innerHTML  = adequacao.map(renderCard).join('');
    document.getElementById('col-concluido').innerHTML  = concluidos.map(renderCard).join('');
    ...
    document.getElementById('count-validacao').textContent  = validacao.length;
    document.getElementById('count-adequacao').textContent  = adequacao.length;
    ...
    document.getElementById('mais-adequacao').style.display  = adequacao.length === 10 ? 'block' : 'none';
    document.getElementById('mais-concluido').style.display  = concluidos.length === 50 ? 'block' : 'none';
  }
  window.carregarCertificacoes = carregarCertificacoes;
    ...
    document.getElementById('detalhe-conteudo').innerHTML = `
      <div style="display:flex;flex-direction:column;gap:16px" class="detail-enter">
        ...
        <div class="detail-grid-2">
          ...
          ${campoDetalhe('Prazo', r.prazo ? formatarPrazo(r.prazo) : '—', 1, 'campo-prazo')}
        </div>
        <div class="detail-grid-2">
        ...
        <div class="detail-field">
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ...
            <button onclick="puxarId()" title="Copiar código"
              style="flex-shrink:0;height:38px;padding:0 10px;border:1px solid var(--gray-200);border-radius:var(--radius-sm);background:var(--white);color:var(--gray-500);cursor:pointer;font-size:14px;display:flex;align-items:center">
  ...
  async function abrirDetalhe(id) {
    ...
    try {
      ...
      renderDetalhe(data.certificacaoCemig || data);
    } catch {
  ...
  function atualizarContadores() {
    ['pendente', 'andamento', 'validacao', 'adequacao', 'concluido'].forEach(col => {
  ...
  async function moverCertificacao(id, novoStatus, colDestino) {
  ...
  function initKanbanDragDrop() {
    ...
    kanban.addEventListener('drop', async (e) => {
      ...
      await moverCertificacao(draggedId, novoStatus, col);
    });
  }
