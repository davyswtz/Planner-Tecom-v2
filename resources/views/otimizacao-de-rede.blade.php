...
window.alterarStatusOS = async function(osId, novoStatus) {
  ...
  async function confirmarExclusaoOs() {
    ...
    try {
      ...
      await window.plannerAposMutacaoLocal(async () => {
        ...
        if (window.carregarOtimizacoes) await window.carregarOtimizacoes();
      });
    } catch (err) {
  ...
  async function confirmarExclusaoTarefa() {
    ...
    try {
      ...
      fecharConfirmacaoExclusao();
      fecharDetalhe();
      await window.plannerAposExclusaoTarefa(id, () => window.carregarOtimizacoes());
    } catch (err) {
...
document.addEventListener('keydown', function(e) {
  ...
  async function salvarEdicao() {
    ...
    const dados = window.plannerDetalheEdicao.montarDados({
      ...
      descricao: { id: 'campo-tipo', tipo: 'textarea' },
    ...
    try {
      await window.plannerDetalheEdicao.enviarPut(`/api/otimizacao-rede/${id}`, dados);
      fecharDetalhe();
...
<script type="module">
  ...
  async function buscarColuna(status, limit, offset = 0, filtros = {}) {
    ...
    const data = await response.json();
    const lista = data.otimizacaoDeRede || [];
